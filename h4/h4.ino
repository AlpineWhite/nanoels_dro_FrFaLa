...
#define PULSE_1_PCNT_UNIT PCNT_UNIT_1 
#define PULSE_2_PCNT_UNIT PCNT_UNIT_2 
...
void taskAttachInterrupts(void *param) {
  ...
  if (PULSE_1_USE){
    startPulseCounter(PULSE_1_PCNT_UNIT, A12, A13, PULSE_1_ENCODER_TYPE, PULSE_1_ENCODER_FILTER);
  }
  if (PULSE_2_USE){
    startPulseCounter(PULSE_2_PCNT_UNIT, A22, A23, PULSE_2_ENCODER_TYPE, PULSE_2_ENCODER_FILTER);
  }
  vTaskDelete(NULL);
}
...
void markAxis0(Axis* a) {
  ...
  if ((PULSE_2_AXIS == a->name) && PULSE_2_USE && PULSE_2_DRO){
    if (xSemaphoreTake(pulse2Mutex, 10) == pdTRUE){
      pcnt_counter_clear(PULSE_2_PCNT_UNIT);
      pulse2Count = 0;
      a->counterDRO = 0;
