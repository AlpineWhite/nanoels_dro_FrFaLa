...
ESP32Encoder* ESP32Encoder::encoders[MAX_ENCODERS] = { nullptr, nullptr, nullptr };
	...
	for (i = 0; i < MAX_ENCODERS; i++) {
		if (intr_status & (BIT(i))) {
			...
			if (PCNT.status_unit[i].COUNTER_H_LIM) {
			    // roll over int32_max
			    if (INT32_MAX - HighLimit >= ptr->count){
			...
			if (PCNT.status_unit[i].COUNTER_L_LIM) {
			    // roll over int32_min
			    if (INT32_MIN - LowLimit <= ptr->count ){
...
void ESP32Encoder::attachEncoder(int i) {
	int index = i;
	fullQuad = type != enc_single; //"fullQuad" is a misnomer should be "notSingle" or "fullOrHalf"
